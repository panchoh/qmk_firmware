...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [L_FN] = LAYOUT_ansi_89(
        ...
        MC_10,    _______,  _______,            _______,  _______,  _______,                       _______,            _______,                      KC_HOME,  KC_PGDN,  KC_END),
    ...
    [WIN_BASE] = LAYOUT_ansi_89(
        ...
        XXXXXXX,  KC_LCTL,  KC_LALT,            KC_LWIN,  KC_SPC,   MO(W_FN),                      KC_SPC,             KC_RALT,                      KC_LEFT,  KC_DOWN,  KC_RGHT),
    ...
    switch (keycode) {
        ...
        case MC_2:
            if (record->event.pressed) {
                if (get_mods() & MOD_MASK_ALT) {
                    clear_mods();
                    SEND_STRING("git checkout -- flake.lock\n");
                } else {
                    clear_mods();
                    SEND_STRING("make bump\n");
                }
            }
            return false; // Skip all further processing of this key
        case MC_3:
            if (record->event.pressed) {
                if (get_mods() & MOD_MASK_ALT) {
                    ...
                    clear_mods();
                    SEND_STRING("doas nixos-rebuild switch\n");
                }
            }
            return false; // Skip all further processing of this key
        case MC_8:
            if (record->event.pressed) {
                if (get_mods() & MOD_MASK_ALT) {
                    clear_mods();
                    SEND_STRING("doas nixos-rebuild test\n");
                } else {
                    clear_mods();
